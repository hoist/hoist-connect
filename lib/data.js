...
DataApi.prototype.remove = function (query, callback) {
  return BBPromise.try(function ensureQuery() {
    if (!query) {
    ...
    return pipeline.remove(this._type, query);
  }).nodeify(callback);
};
DataApi.prototype.removeOneById = function (id, callback) {
  return BBPromise.try(function ensureQuery() {
    if (!id) {
    ...
    return pipeline.remove(this._type, {
      _id: id
    });
  }).nodeify(callback);
};
