...
var contextResolver = require('hoist-context');
...
ModelLogger.prototype.log = function (message) {
  ...
  new Model.ExecutionLogEvent(logEvent).saveAsync()
    .bind(this)
    .then(function logSave(logEvent) {
      hoistLogger.info({
        saved: logEvent
      }, 'custom log saved ok');
    }).catch(function errorLog(err) {
...
function createClient(context) {
  ...
  if (context.application && context.application.loggly) {
    loggers.push(loggly.createClient({
      ...
      tags: ['Hoist', context.application.organisation.slug, context.application.slug, context.application.organisation.slug + '/' + context.application.slug]
    }));
  }
...
module.exports = function () {
  ...
  getClient = contextResolver.get().then(function (context) {
