...
var UserAPI = (function (_BaseAPI) {
  function UserAPI() {
    ...
    this.pipeline = new _hoistUserPipeline2['default']();
  }
  ...
  _createClass(UserAPI, [{
    ...
    value: function login(username, password, callback) {
      ...
      return _bluebird2['default'].resolve(this.pipeline.login(username, password)).nodeify(callback);
    }
  }, {
    key: 'invite',
    value: function invite(userDetails, callback) {
      ...
      return _bluebird2['default'].resolve(this.pipeline.invite(userDetails)).nodeify(callback);
    }
  }]);
